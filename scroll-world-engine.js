...
function mountScrollWorld(container, config) {
  ...
  const SECTION_CROSSFADE = config.sectionCrossfades || [];
  const seamFade = i => (SECTION_CROSSFADE[i] != null ? SECTION_CROSSFADE[i] : CROSSFADE);
  ...
  SEGMENTS.forEach(s => {
    ...
    scene.classList.add(`sw-scene--${s.kind}`, `sw-scene--section-${s.si}`);
  ...
  function layout() {
    ...
    track.style.height = (totalW * vh + EXIT_RUNWAY * vh) + 'px';
    read();
  }
  ...
  function loadClip(s) {
    ...
    function attachVideo(src) {
        ...
        v.addEventListener('error', () => { s.loading = false; s.ready = false; });
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // The last scene holds briefly after its camera move completes, then clears
      // before the following page content enters.
      if (i === NSEG - 1 && y > s.end) outside = Math.max(0, y - (s.end + EXIT_HOLD * vh));
      const fade = Math.max(1, (y < s.start ? s.fadeIn : s.fadeOut) * vh);
      const op = smooth(1 - outside / fade);
      s.el.style.opacity = op; s.visible = op > 0.001;
      // Keep later story beats above earlier ones. The incoming scene can then
      // genuinely dissolve over the outgoing frame during its fade-in window;
      // promoting only the current segment would hide that blend and produce a cut
      // at the exact seam.
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (i === 0) cop = after ? 0 : smooth(1 - pr / 0.62);            // greets on landing
        ...
        if (before) cop = 0;
        else if (after) cop = smooth(1 - (y - (seg.end + EXIT_HOLD * vh)) / (EXIT_FADE * vh));
        else cop = smooth(pr / 0.4);
      }
      else cop = (before || after) ? 0 : smooth(1 - Math.abs(pr - 0.5) / 0.5);
      const c = copies[i];
      c.style.opacity = cop;
      const copyDrift = (0.5 - pr) * 4;
      c.style.transform = reduce
        ? (window.innerWidth > 860 ? 'translateY(-50%)' : 'none')
        : (window.innerWidth > 860
          ? `translateY(calc(-50% + ${copyDrift}vh))`
          : `translateY(${copyDrift}vh)`);
    ...
    const worldChrome = smooth(1 - (y - (totalW * vh + EXIT_HOLD * vh)) / (EXIT_FADE * vh));
  ...
  function ctaBtns(cta) {
    let h = '';
    const attrs = href => /^https?:\/\//i.test(href) ? ' target="_blank" rel="noopener"' : '';
    if (cta.primary) {
      const href = cta.primary.href || '#';
      h += `<a class="sw-btn sw-btn--primary" href="${esc(href)}"${attrs(href)}>${esc(cta.primary.label)}</a>`;
    }
    if (cta.secondary) {
      const href = cta.secondary.href || '#';
      h += `<a class="sw-btn sw-btn--ghost" href="${esc(href)}"${attrs(href)}>${esc(cta.secondary.label)}</a>`;
    }
    return h;
  }
}
